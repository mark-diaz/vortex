...
`include "cmd_dispatch_mod.sv"
`include "ccip_read_req.sv"
`include "ccip_read_req_mod.sv"
...
module vortex_afu import ccip_if_pkg::*; import local_mem_cfg_pkg::*; import VX_gpu_pkg::*; #(
    ...
    // Final_Project
    reg [63:0] io_addr_packet, data_size_packet;
    t_ccip_clAddr io_addr = t_ccip_clAddr'(io_addr_packet);
    wire [CCI_ADDR_WIDTH-1:0] data_size = CCI_ADDR_WIDTH'(data_size_packet);


    `UNUSED_VAR (io_addr_packet);
    `UNUSED_VAR (data_size_packet);
...
`endif
    ...
    always @(posedge clk) begin
        if (cp2af_sRxPort.c0.mmioWrValid) begin
            ...
            MMIO_TEST_FLUSH: begin
                io_addr_packet <= 64'(cp2af_sRxPort.c0.data);

            `ifdef DBG_TRACE_AFU
                `TRACE(2, ("%t: AFU: MMIO_TEST_FLUSH: data=0x%h \n", $time, 64'(cp2af_sRxPort.c0.data)))
            `endif
            end
            ...
            `ifdef DBG_TRACE_AFU
                `TRACE(2, ("%t: AFU: MIO_CMD_ARG1: data=0x%h\n", $time, 64'(cp2af_sRxPort.c0.data)))
            `endif
            end
            ...
            MMIO_CMD_ARG2: begin
                ...
                // Final_Project
                data_size_packet <= 64'(cp2af_sRxPort.c0.data);
    ...
    reg  cmd_mem_wr_done = '0; // Final_Project temp fix
...
*/
   ...
   // AVS Controller /////////////////////////////////////////////////////////////
    ...
    // CCI-P Read Request - 2 ///////////////////////////////////////////////////////////


    reg [CCI_ADDR_WIDTH-1:0] cci_mem_wr_req_ctr = '0;
    wire [CCI_ADDR_WIDTH-1:0] cci_mem_wr_req_addr = '0;
    reg [CCI_ADDR_WIDTH-1:0] cci_mem_wr_req_addr_base = '0;
    ...
    wire [CCI_DATA_WIDTH-1:0] cci_rdq_din_packet, cci_rdq_dout_packet;
    wire cci_rdq_empty;

    // Check if CCIP Read Request is valid
    ...
    reg placeholder = 1'b0;
    assign cci_rdq_pop  = placeholder;
    assign cci_rdq_din_packet = cp2af_sRxPort.c0.data;


    // Temporary unused
    assign cci_rdq_dout = '0;
    `UNUSED_VAR(cci_rdq_dout);
    `UNUSED_VAR(cci_mem_wr_req_addr_base);
    `UNUSED_VAR(cci_rdq_dout_packet);
    `UNUSED_VAR(cci_rdq_empty);
    //assign cci_mem_wr_req_valid = !cci_rdq_empty; <--- Assign to HWQ
    assign cci_mem_wr_req_valid = '0;
    assign cci_mem_wr_req_addr = '0;
    ...
    VX_fifo_queue #(
        .DATAW (CCI_DATA_WIDTH), // $bits(t_ccip_clData)
        .DEPTH (CCI_RD_QUEUE_SIZE) // 16 by default
    ) cci_rd_req_queue (
        .clk      (clk),
        .reset    (reset),
        .push     (cci_rdq_push),
        .pop      (cci_rdq_pop),
        .data_in  (cci_rdq_din_packet),
        .data_out (cci_rdq_dout_packet),
        .empty    (cci_rdq_empty),
        `UNUSED_PIN (full),
...
`ifdef DBG_TRACE_AFU
  /*
    always @(posedge clk) begin
                `TRACE(2, ("%t: Debugging: data=%d ended=%d\n", $time, cp2af_sRxPort.c0.data, ended);
    end
    */
`endif
    ...
    // Note: Doesn't Read Data
    // ==> Data is being inputed to FIFO_QUEUE
    // ==> Instead, it controls the data being inputted into the FIFO QUEUE
    ccip_read_req #(
